...
public class AWToolsTest {
	...
	public void testStringToDate() {
		...
		try {
			Date convert = AWTools.stringToDate(stringDate1, "dd.MM.yyyy HH:mm:ss");

			log.debug("Input: " + stringDate1);
			log.debug("Equal: " + DateFormat.getDateInstance().format(greg1.getTime()));
		...
		try {
			Date convert = AWTools.stringToDate(stringDate2, "dd.MM.yyyy HH:mm:ss");

			log.debug("Input: " + stringDate2);
			log.debug("Equal: " + DateFormat.getDateInstance(DateFormat.LONG).format(greg2.getTime()));
			log.debug("Parse: " + convert);
			log.debug("Conv : " + DateFormat.getDateInstance().format(convert));
	...
	public void testFormatDouble() {
		double[] value = new double[] { 100.01, 100.001, 89.3, 5555.5555, 12.1, 13.12, 5.5 };

		String[] conv = new String[] { "100,01", "100,00", "89,30", "5.555,56", "12,10", "13,12", "5,50" };
	...
	public void testReplacePlaceholder() {
		...
		assertThat(AWTools.replacePlaceholder(test, placeholder)).isEqualTo("Hallo Andre! Wie war der ${heute}?");

		placeholder.put("heute", "Sonntag");
		assertThat(AWTools.replacePlaceholder(test, placeholder)).isEqualTo("Hallo Andre! Wie war der Sonntag?");

		test = "Ganz normaler String";
		assertThat(AWTools.replacePlaceholder(test, placeholder)).isEqualTo(test);
	}
